...
def get_Trie(output_file,min_len=2):
    '''
    构建实体字典树，长度小于2的不插入
    :param min_len: 实体长度
    :return:
    '''
    kb_en = pd.read_pickle(output_file)
    trie_obj = Trie()
    for en in kb_en:
        if len(en)>=min_len:
            trie_obj.insert(en)
...
def get_split_entity(train_file,entiy_id_file,output_file):
    '''
    ...
    '''
    def entity_split():
        trie_obj = get_Trie(entiy_id_file)
        entity_count = {}
...
if __name__ == '__main__':
    train_file = "../original_data/train.json"
    entiy_id_file ="../data/entity_id.pkl"
    output_file ="../data/split_entity.pkl"
    get_split_entity(train_file,entiy_id_file,output_file)
    # trie_obj = get_Trie()
